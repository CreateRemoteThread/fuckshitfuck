...
def deriveKey(data,plaintexts):
  ...
  if TRACE_LENGTH == 0:
    TRACE_LENGTH = data[0].size
  print("DERIVING FOR TRACE LENGTH %d" % TRACE_LENGTH)
  for BYTE_POSN in range(0,16):
    ...
    for KEY_GUESS in range(0,256):
      ...
      if max(diffProfile) > maxDiff:
        ...
        globalMax = abs(diffProfile)
    # x = argsort(globalMax)[-3:]
    x = magicalBackoffSort(globalMax)
    x_avg = average(globalMax)
    gm = zeros(TRACE_LENGTH)
    gm.fill(x_avg)
    # for i_m in range(0,TRACE_LENGTH):
    for i_m in x:
      # difftest = abs(globalMax[i_m] - x_avg)
      # if difftest >= x_avg:
      gm[i_m] = abs(globalMax[i_m] - x_avg)
      # plt.annotate_
      print("Assigning value %f to %d" % (abs(globalMax[i_m] - x_avg), i_m))
    print("Average for round: %f" % x_avg)
    plt.plot(list(range(0,TRACE_LENGTH)),gm)
