...
def deriveKey(data,plaintexts,usefulTraceLength):
  ...
  for BYTE_POSN in range(0,16):
    for KEY_CAND in range(0,256):  
      for TRACE_IND in range(0,len(plaintexts)):
        pt = binascii.unhexlify(plaintexts[TRACE_IND])
        # for each byte posn, and each key candidate, do:
        # hweight(sbox(plaintext[posn] ^ key_candidate))
        temp = ord(pt[BYTE_POSN]) ^ KEY_CAND
        varray[TRACE_IND,KEY_CAND,BYTE_POSN] = bin(sbox[temp]).count("1")
